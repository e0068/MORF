...
The full model is on the canvas at `.morf/model.canvas`;
...
2. Read this memory's levels: `.morf/Observations/L3.md`, `L2.md`,
   `L1.md`. Do not read `L0.md` — it is input for consolidation, not memory.

3. Read `.morf/levels.md`: it lists the levels, their horizons and
...
4. If the first level changed or is approaching its limit, consolidate
   ...
   - recompute scores: `python3 "$CLAUDE_PROJECT_DIR/.morf/scripts/score-memory.py"`
   ...
   - report what changed in a single table: `python3 "$CLAUDE_PROJECT_DIR/.morf/scripts/trace.py" --show`
...
5. What the memory owes arrives on **every** turn, not once at the start: an
   ...
   work or say in the answer that you left it, and why. `python3 "$CLAUDE_PROJECT_DIR/.morf/scripts/due.py"`
   prints what this memory owes; one `.morf` is one memory, so there is nothing
   else to sweep.
...
- Putting a rule into a file is three steps, and the last two are yours to run,
  ...
  `python3 "$CLAUDE_PROJECT_DIR/.morf/scripts/rules.py" --track <file>` if that file
...
- When a watched file drifts, discharge it in this order: read
  `python3 "$CLAUDE_PROJECT_DIR/.morf/scripts/rules.py" --diff`, write what changed
...
- A folder name states its relation to its parent. Read a path bottom-up as
  a phrase before creating it: `.morf/Observations/Observations` would read as
  "observations of observations", which is not a thing.
...
- Never write to or delete from `.morf/Transcripts`. It is filled by
