...
**How it works.** Two hops: `Claude/Memory/TAGS.md` → the relevant part of `INDEX.md` → 2–5 articles. Both indexes are generated by `build-index.py`.
...
**Path check.** A folder name states its relation to its parent, not a standalone label. A path is read bottom-up as a phrase: `Claude/Memory/Transcripts/<id>` reads as transcripts of Claude's memory and holds together; `Claude/Memory/Memory` would read as "memory of memory" and does not. `build-index.py` reports repeated names on every build. The same test catches the opposite mistake — a level that adds nothing, like `Projects/shader-lab/project.md`.
